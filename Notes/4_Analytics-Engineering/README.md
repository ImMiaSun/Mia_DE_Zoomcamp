...
- References raw data that's loaded into your data warehouse but not yet transformed by dbt
...
The second source of data we select from is **seeds**. Seeds are <u>CSV files</u> stored within our repository. When running dbt run on a seed, DBT executes an operation similar to a COPY INTO command in SQL.

Imagine seeds as **small Excel/CSV files** that you store right alongside your dbt code. These files contain:

- Reference data (like country codes)
- Mapping tables (like product categories)
- Small datasets that rarely change (like store locations)
...
- References another dbt model (a transformed table/view)
...
select
    -- identifiers
    {{ dbt_utils.generate_surrogate_key(['vendorid', 'lpep_pickup_datetime']) }} as tripid, -- combine 2 columns
...
To test this, you can run the code as it is and confirm that the LIMIT 100 is applied. If you don’t want the limit, <u>you can override the variable during execution by passing a dictionary of variables</u>. For example:
...
There are a few differences: the staging file typically includes a straightforward CTE (common table expression - only exists during query session) and doesn't use the same variables as before. However, the core SQL logic remains largely the same.
...
with green_tripdata as (
    select *, 
        'Green' as service_type 
  			-- Adds a new column called service_type with the constant value 'Green'
...
This model creates a table with monthly revenue metrics <u>per pickup zone and service type</u>, including various fare components, trip counts, and averages.
...
`severity: warn` -- Allows the run to continue but logs the failure
...
If any `pickup_locationid` does **not** exist in `taxi_zone_lookup`, it will log a **warning**
...
development. We’ve already seen how to handle development, testing, and documentation, all happening in our development environment.

Now, to take it into production, we go through a process called deployment. This involves taking all our code, 
opening a **pull request** (*proposing and reviewing changes* before merging them), and merging the code into the main branch, which affects our production environment. 
...
production environment. By default, it includes a dbt build step, but this can be customized. Within the deployment process, we can create dbt jobs that run multiple commands. A single job can execute multiple steps,
 such as *dbt build*, *dbt test*, *dbt seed*, or *dbt source freshness*. These runs can be triggered manually, on a scheduled basis (e.g., using a cron schedule), or via an API.
...
In a real-world scenario, this is crucial for ensuring reliability and visibility. For instance, we can perform
 a complete dbt build, generate documentation for production, and ensure that documentation is accessible to everyone on the team. We can also run `dbt source freshness` to validate source data.

The scheduled runs can be set up to occur daily at a specific time, excluding weekends if no data is received on Saturdays or Sundays. Advanced settings include options like timeout configurations and the number of models 
